...
class StyleGANDataset(Pix2pixDataset):
    ...
    def initialize(self, opt):
        self.opt = opt
        label_paths, app_paths, image_paths, instance_paths = self.get_paths(opt)
    ...
    def get_paths(self, opt):
        ...
        label_paths = [os.path.join(opt.dataroot, 'crop_Img', x) for x in annotations['pose_image']]
        app_paths = [os.path.join(opt.dataroot, 'crop_Img', x) for x in annotations['app_image']]
        images_paths = [os.path.join(opt.dataroot, 'crop_Img', x) for x in annotations['target_image']]
        if not opt.no_instance:
