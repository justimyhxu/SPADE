...
class Pix2PixTranModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.netG, self.netD, self.netE_app, self.netE_pose = self.initialize_networks(opt)
    ...
    def compute_generator_loss(self, input_images, real_image):
        ...
        if self.opt.use_vae:
            G_losses['KLD_pose'] = KLD_loss_pose
            G_losses['KLD_app'] = KLD_loss_app

        pred_fake, pred_real = self.discriminate(
            input_images, fake_image, real_image)
    ...
    def encode_z(self, real_image, net):
        mu, logvar = net(real_image)
        z = self.reparameterize(mu, logvar)
        return z, mu, logvar

    def generate_fake(self, app_image, real_image, compute_kld_loss=False, pose_image=None):
        z = None
        KLD_loss_pose, KLD_loss_app = None, None
        if self.opt.use_vae:
            z_pose, mu_pose, logvar_pose = self.encode_z(pose_image, self.netE_pose)
        ...
        return fake_image, KLD_loss_pose, KLD_loss_app
